-- 리뷰 작성 스키마 델타 — V15에 사후 수정으로 들어갔던 변경을 새 버전으로 분리.
-- 리뷰는 order_item로만 연결하고 옵션명은 스냅샷으로 보존하므로 reviews의 조합 FK 컬럼을 제거한다.
SET @stmt = IF(
    EXISTS(
        SELECT 1 FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE()
          AND TABLE_NAME = 'reviews'
          AND COLUMN_NAME = 'option_combination_id'
    ),
    'ALTER TABLE reviews DROP COLUMN option_combination_id',
    'SELECT 1'
);
PREPARE drop_option_combination_stmt FROM @stmt;
EXECUTE drop_option_combination_stmt;
DEALLOCATE PREPARE drop_option_combination_stmt;
