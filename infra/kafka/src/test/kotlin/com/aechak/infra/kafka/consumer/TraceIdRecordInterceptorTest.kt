...
class TraceIdRecordInterceptorTest {
    ...
    fun `헤더의 traceId를 MDC로 복원한다`() {
        ...
        assertEquals("trace-abc", MDC.get("traceId"), "발행 홉이 실어 보낸 traceId가 리스너 스레드 MDC로 이어져야 한다")
    }
    ...
    fun `헤더가 없으면 새 traceId를 만들어 체인의 뿌리로 삼는다`() {
        ...
        assertTrue(MDC.get("traceId").isNotBlank(), "헤더 없는 레코드도 이 지점부터의 체인은 묶여야 한다")
    }
    ...
    fun `처리가 끝나면 MDC를 지운다`() {
        ...
        assertNull(MDC.get("traceId"), "리스너 스레드는 풀에서 재사용되므로 이전 레코드의 traceId가 남으면 안 된다")
    }
